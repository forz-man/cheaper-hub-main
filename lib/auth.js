...
function oauthCallbackUrl(role, { signup = false } = {}) {
...
function getClient() {
  if (!supabase) {
    console.warn(
      "Supabase client is not initialized. Check your NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY secrets.",
    );
    return {
      auth: {
        signUp: async () => ({
...
export async function register({ email, password, fullName, role }) {
  return await getClient().auth.signUp({
    ...
    options: {
      ...
      emailRedirectTo: oauthCallbackUrl(role, { signup: true }),
    },
  });
}
...
export async function loginWithGoogle(role, options = {}) {
  const redirectTo = oauthCallbackUrl(role, options);
...
export async function loginWithApple(role, options = {}) {
  const redirectTo = oauthCallbackUrl(role, options);

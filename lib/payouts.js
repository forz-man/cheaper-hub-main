...
const PAYOUT_RECOVERY_REQUEST_TIMEOUT_MS = 30_000;
// Leave enough time for the bounded Stripe request to return and its owner to
// persist a result before a redelivered webhook reconciles an abandoned claim.
const PAYOUT_RECOVERY_STALE_ATTEMPT_MS = 120_000;
...
async function createOrReuseRecoveryEvent({ eventRecord, allocation, order, event, admin }) {
  ...
  return {
    recovery: data,
    ownsAttempt: data?.status === "processing" && data.attempt_token === attemptToken,
  };
}
...
async function finalizePayoutRecovery({
  recoveryId,
  attemptToken,
  status,
...
async function reconcileStalePayoutRecovery({
  recoveryId,
  stripeTransferReversalId = null,
  errorMessage = null,
  admin,
}) {
  const { data, error } = await admin.rpc("reconcile_stale_payout_recovery", {
    p_recovery_id: recoveryId,
    p_stale_after_seconds: Math.floor(PAYOUT_RECOVERY_STALE_ATTEMPT_MS / 1000),
    p_stripe_transfer_reversal_id: stripeTransferReversalId,
    p_error: errorMessage,
  }).single();
  if (error) throw error;
...
function isRecoveryAttemptStale(recovery) {
  const startedAt = Date.parse(recovery?.attempt_started_at || "");
  return Number.isFinite(startedAt) && Date.now() - startedAt >= PAYOUT_RECOVERY_STALE_ATTEMPT_MS;
}

async function findExistingTransferReversal({ recovery, stripeClient }) {
  let startingAfter;
  do {
    const page = await stripeClient.transfers.listReversals(
      recovery.stripe_transfer_id,
      { limit: 100, ...(startingAfter ? { starting_after: startingAfter } : {}) }
    );
    const existing = (page.data || []).find((reversal) =>
      reversal?.metadata?.stripe_adjustment_id === recovery.stripe_adjustment_id ||
      reversal?.metadata?.stripe_event_id === recovery.stripe_event_id
    );
...
export async function processStripeRefundOrDispute({
  ...
  for (const allocation of allocations) {
    let { recovery, ownsAttempt } = await createOrReuseRecoveryEvent({
      eventRecord,
      allocation,
      order,
      event,
    ...
    if (
      ...
      !ownsAttempt &&
      isRecoveryAttemptStale(recovery)
    ) {
      try {
        const existingReversal = await findExistingTransferReversal({ recovery, stripeClient });
        recovery = await reconcileStalePayoutRecovery({
          recoveryId: recovery.id,
          stripeTransferReversalId: existingReversal?.id || null,
          errorMessage: existingReversal
            ? null
            : "Previous payout recovery worker did not complete; safely retrying with the same Stripe idempotency key.",
          admin,
        });
        if (recovery?.status === "needs_support") {
          ({ recovery, ownsAttempt } = await createOrReuseRecoveryEvent({
            eventRecord,
            allocation,
            order,
            event,
        ...
        const message = err?.message || "Unable to reconcile an interrupted seller payout recovery.";
    ...
    if (["recovered", "reinstated", "not_applicable"].includes(recovery?.status)) {
      if (recovery.status === "recovered") recoveredCount += 1;
      continue;
    }
    if (!ownsAttempt) {
      retryPending = true;
      warnings.push(`Item ${allocation.id}: seller payout recovery is already in progress; retrying later.`);
      continue;
    }
    ...
    try {
      const reversal = await stripeClient.transfers.createReversal(
        ...
        },
        {
          ...
          timeout: PAYOUT_RECOVERY_REQUEST_TIMEOUT_MS,
        }
      );

      await finalizePayoutRecovery({
        recoveryId: recovery.id,
        attemptToken: recovery.attempt_token,
        status: "recovered",
  ...
  const nextOrderValues = retryPending
    ? {}
    : isRefund
  ...
  const eventStatus = retryPending ? "processing" : warnings.length > 0 ? "needs_support" : "processed";
  ...
  return {
    processed: !retryPending && warnings.length === 0,
    retryRequired: retryPending || warnings.length > 0,
    orderId: order.id,
    recoveredCount,
    warnings,
  };

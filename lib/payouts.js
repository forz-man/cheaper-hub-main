 ...
 *
 * For Escrow orders: Escrow.com handles the actual money movement on their
 * platform. We only update our DB to reflect the release.
 */
...
export async function attemptPayoutRelease(orderId) {
  ...
  const { data: order, error: orderErr } = await admin
    .from("orders")
    .select(
      "id, buyer_confirmed_at, payment_status, payment_method, stripe_payment_intent, payouts_released_at"
    )
    .eq("id", orderId)
    .single();
  ...
  for (const item of items) {
    try {
      ...
      if (order.payment_method === "stripe" && order.stripe_payment_intent && item.vendor_id) {
      ...
      // Escrow orders: money movement is handled by Escrow.com; we just
      // mark our DB row as released.

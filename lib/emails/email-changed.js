...
export function renderEmailChangedEmail({ oldEmail, newEmail, changedAt }) {
  return `
<!DOCTYPE html>
<html>
          ...
          <tr>
            <td style="padding:8px 32px 24px 32px;">
              <p style="margin:0;font-size:15px;line-height:1.6;color:#71717A;">
                A request was made on ${changedAt} to change the email address on your Cheaper account from
                <strong style="color:#18181B;">${oldEmail}</strong> to
                <strong style="color:#18181B;">${newEmail}</strong>. The change is not complete until the required confirmation link is used.
              </p>
            </td>
          </tr>
          <tr>
            <td style="padding:0 32px 32px 32px;">
              <p style="margin:0;font-size:14px;line-height:1.6;color:#18181B;background-color:#F4F4F5;border-radius:8px;padding:16px;">
                Wasn't you? Contact <a href="mailto:support@cheaper.com" style="color:#16A34A;font-weight:600;text-decoration:none;">support@cheaper.com</a> immediately.
              </p>
            </td>
          </tr>
...
export async function sendEmailChangedNotification({ oldEmail, newEmail, changedAt }) {
  const resend = getResend();
  const html = renderEmailChangedEmail({ oldEmail, newEmail, changedAt });
  const subject = "Confirm your Cheaper email change";

  // Notify both addresses: old one (in case this wasn't the account owner)
  // and new one (confirming this is now where notifications go).
  return Promise.all([
    resend.emails.send({ from: EMAIL_FROM, to: oldEmail, subject, html }),
    resend.emails.send({ from: EMAIL_FROM, to: newEmail, subject, html }),
  ]);
}

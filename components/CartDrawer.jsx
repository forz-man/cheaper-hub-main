          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#e2ddd6] flex-shrink-0">
              ...
              <div className="flex items-center gap-3">
                {items.length > 0 && (
                  <button
                    onClick={clearCart}
                    className="text-xs text-[#aaa] hover:text-red-500 transition-colors font-medium cursor-pointer font-bold outline-none"
                  >
                    Clear all
                  </button>
                )}
                <button
                  onClick={closeCart}
                  className="w-8 h-8 rounded-full bg-[#f5f3ef] flex items-center justify-center hover:bg-[#e2ddd6] transition-colors cursor-pointer outline-none"
                >
            ...
            <div className="flex-1 overflow-y-auto">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full px-8 text-center">
                  ...
                  <button
                    onClick={closeCart}
                    className="bg-[#111] text-white px-6 py-2.5 rounded-lg text-sm font-semibold hover:bg-[#333] transition-colors cursor-pointer outline-none"
                  >
                    Keep shopping
                  </button>
                </div>
                ...
                <div className="divide-y divide-[#f0ede8]">
                  {items.map((item) => (
                    <div key={item.id} className="flex gap-4 px-5 py-4 animate-fadeIn">
                      <div className="w-14 h-14 rounded-xl bg-[#f5f3ef] border border-[#e2ddd6] flex items-center justify-center flex-shrink-0 overflow-hidden">
                        {item.images?.[0] ? (
                          <img src={item.images[0]} alt="" className="w-full h-full object-contain p-1.5" />
                        ) : (
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center border border-[#e2ddd6] rounded-lg overflow-hidden">
                            <button
                              ...
                              className="w-7 h-7 flex items-center justify-center text-[#888] hover:bg-[#f5f3ef] transition-colors cursor-pointer outline-none"
                            >
                              <Minus size={11} />
                            </button>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <button
                              ...
                              className="text-[#ccc] hover:text-red-500 transition-colors cursor-pointer outline-none"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
            ...
            {items.length > 0 && (
              <div className="border-t border-[#e2ddd6] px-5 py-5 space-y-3 flex-shrink-0 bg-white">
                ...
                <button
                  onClick={closeCart}
                  className="w-full text-center text-sm text-[#888] hover:text-[#111] transition-colors py-1 font-medium cursor-pointer outline-none"
                >
                  Continue shopping
                </button>
              </div>
